...
Figure~\ref{fig:nvc} shows the two-phase NVC used to rectify one of the three two-phase groups introduced in the overview of this chapter, each combining a pair of pickup units of \eqref{eq:phase_voltage} at the terminals $A$ and $B$. The bridge replaces the diodes of the FWR with four MOSFETs $S_1$--$S_4$, each with a Schottky diode ($D_1$--$D_4$) connected in parallel. $S_1$ and $S_2$ connect terminal $A$ to $\text{OUT+}$ and $\text{OUT-}$ respectively, while $S_3$ and $S_4$ connect terminal $B$ to $\text{OUT+}$ and $\text{OUT-}$ respectively. The bridge is \emph{gate cross-coupled}: the gates of $S_1$ and $S_2$ are tied together and driven directly by terminal $B$, while the gates of $S_3$ and $S_4$ are tied together and driven directly by terminal $A$. No separate gate-drive or polarity-detection circuit is needed, since each pair of switches is driven by the very AC terminal it is not connected to.
