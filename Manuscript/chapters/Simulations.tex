...
\subsubsection{First and Second Bridges (FWR1, $k=0,2,4$; FWR2, $k=1,3,5$)}
...
Figures \ref{fig:fwr_star_pce} and \ref{fig:fwr_star_pee} show the power conversion efficiency (PCE, \eqref{eq:pce}) and the power extraction efficiency (PEE, \eqref{eq:pee}) for the same six values of $C_m$ across the speed range. The PCE curves are much less sensitive to the choice of $C_m$ than the PEE curves: all six converge towards $\text{PCE}\approx0.7$--$0.75$ by $400\ \text{rpm}$, and even at $100\ \text{rpm}$ they stay within a comparatively narrow band, regardless of $C_m$. The PEE curves, by contrast, are highly sensitive to $C_m$: each one peaks close to $\text{PEE}=1$ at the single speed for which that value of $C_m$ is closest to \eqref{eq:matching_cap}, and falls off on either side, mirroring the crossover already seen in the frequency sweep of Figure \ref{fig:fwr_star_freq_power}. This is a direct consequence of \eqref{eq:pee}, $\text{PEE}=(P_\text{in}/P_\text{max})\cdot\text{PCE}$: since PCE stays roughly constant, most of the variation in PEE comes from $P_\text{in}/P_\text{max}$, that is, from how far $C_m$ is from cancelling the coil's reactance at that speed, rather than from the bridge's own conversion loss.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{Plots/Y_C_sweep/Y_Eff_sweep.pdf}
    \caption{Power conversion efficiency (PCE) of the first and second bridges combined, across the target speed range, for six fixed values of the compensation capacitor $C_m$ ($C_r=1480\ \mu\text{F}$, $R_L=14.25\ \Omega$).}
