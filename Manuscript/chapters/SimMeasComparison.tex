...
The three configurations diverge in how their measured output power compares to simulation across the range. The star connection's measured power stays at or above the simulated value at every speed, with the two converging almost exactly from $250\ \text{rpm}$ onward. The delta connection starts the same way at $100\ \text{rpm}$, but crosses below simulation by $200\ \text{rpm}$ and stays there. The NVC, by contrast, is the only configuration whose measured power is below simulation across the whole range, with the widest relative gap of the three appearing around $300\ \text{rpm}$. The rectifier efficiency is more consistent among configurations: all three are measured below simulation for most of the range, and the star connection's efficiency gap in particular grows steadily with speed, consistent with a loss mechanism, such as core or frequency-dependent conductor losses, that is absent from the frequency-independent coil model of Chapter \ref{ch:simulations} and matters more at high speed. The NVC's efficiency gap instead peaks in the middle of the range and narrows at both ends, which points less to a single loss mechanism than to how closely a $C_m$ fixed at its $100\ \text{rpm}$ value continues to match the coil as speed changes.

The NVC's gap has a further component that this $C_m$ mismatch does not explain: even at $100\ \text{rpm}$, its own design point, measured power and efficiency already sit below simulation ($57.7$ against $69.0\ \text{mW}$; $0.568$ against $0.665$). The Simulink model of Section \ref{sec:sim_nvc} uses the MOSFET on-resistance and threshold voltages of Table \ref{tab:board_components}, but those are nominal datasheet figures; the individual, hand-soldered devices on the prototype inevitably deviate from them by some manufacturing tolerance, and on-resistance rises further with temperature during operation. Either effect adds conduction loss beyond what the model assumes, independently of speed, consistent with a gap already present at the design point itself, before any $C_m$ mismatch develops.

The anti-series NVC configuration further assumes the two paired phases are electrically $180^\circ$ apart, so that their voltages combine without any partial cancellation (Eq.~\eqref{eq:antiseries}). Section \ref{sec:sim_overview} already reports that manufacturing tolerances between the six nominally identical pickup units cause their peak voltages to differ (Table \ref{tab:sim_voltage}); the same tolerances make it unlikely that the real electrical spacing between paired phases is exactly $180^\circ$, so their anti-series combination falls somewhat short of the ideal doubling assumed in simulation. The FWR groupings, which sum three phases $120^\circ$ apart rather than relying on a single antiphase cancellation, are comparatively less sensitive to this kind of spacing error, which may help explain why this baseline gap shows up more clearly on the NVC than on either FWR connection.
